...
\var_dump($grammar1->parse('a') === [['a']]);

// new S(new A("a", "b"))
$grammar1 = $locoGrammar->parse(" S ::= A \n A ::= 'a' 'b' ");
\var_dump($grammar1->parse('ab') === [['a', 'b']]);
...
\var_dump($grammar1->parse('a') === ['a']);
\var_dump($grammar2->parse('a') === ['a']);
\var_dump($grammar3->parse('a') === [['a']]);
\var_dump($grammar4->parse('a') === [['a']]);
\var_dump($grammar1->parse('') === []);
\var_dump($grammar2->parse('') === []);
\var_dump($grammar3->parse('') === [[]]);
\var_dump($grammar4->parse('') === [[]]);
...
\var_dump($grammar1->parse('aaa') === ['a', 'a', 'a']);
\var_dump($grammar4->parse('aaa') === ['a', 'a', 'a']);
\var_dump($grammar2->parse('aaa') === [['a', 'a', 'a']]);
\var_dump($grammar3->parse('aaa') === [['a', 'a', 'a']]);
\var_dump($grammar1->parse('aa') === ['a', 'a']);
\var_dump($grammar4->parse('aa') === ['a', 'a']);
\var_dump($grammar2->parse('aa') === [['a', 'a']]);
\var_dump($grammar3->parse('aa') === [['a', 'a']]);
\var_dump($grammar1->parse('a') === ['a']);
\var_dump($grammar4->parse('a') === ['a']);
\var_dump($grammar2->parse('a') === [['a']]);
\var_dump($grammar3->parse('a') === [['a']]);
...
\var_dump($grammar1->parse('ababab') === ['ababab']);
$grammar = $locoGrammar->parse(' number ::= /a\\.b/ ');
\var_dump($grammar->parse('a.b') === ['a.b']);
...
\var_dump($grammar1->parse("\x41") === ["\x41"]); # 1-byte character "A"
\var_dump($grammar1->parse("\xC2\xAF") === ["\xC2\xAF"]); # 2-byte character "�"
\var_dump($grammar1->parse("\xE2\x99\xA5") === ["\xE2\x99\xA5"]); # 3-byte character "?"
\var_dump($grammar1->parse("\xF1\x8B\x81\x82") === ["\xF1\x8B\x81\x82"]); # 4-byte character "??"
\var_dump($grammar1->parse("\xEF\xBB\xBF") === ["\xEF\xBB\xBF"]); # "byte order mark" 11101111 10111011 10111111 (U+FEFF)
\var_dump($grammar1->parse("\xF0\x90\x80\x80") === ["\xF0\x90\x80\x80"]); # 4-byte character
\var_dump($grammar1->parse("\xF0\xA0\x80\x80") === ["\xF0\xA0\x80\x80"]); # 4-byte character
try {
    $grammar1->parse("\xF4\x90\x80\x80");
    \var_dump(false);
} catch (ParseFailureException $e) {
    \var_dump(true);
} # code point U+110000, out of range (max is U+10FFFF)
try {
    $grammar1->parse("\xC0\xA6");
    \var_dump(false);
} catch (ParseFailureException $e) {
    \var_dump(true);
} # overlong encoding (code point is U+26; should be 1 byte, "\x26")
try {
    $grammar1->parse("\xC3\xFF");
    \var_dump(false);
} catch (ParseFailureException $e) {
    \var_dump(true);
...
try {
    $grammar1->parse("\xED\xA0\x80");
    \var_dump(false);
} catch (ParseFailureException $e) {
    \var_dump(true);
} # 55296d
try {
    $grammar1->parse("\xED\xBF\xBF");
    \var_dump(false);
} catch (ParseFailureException $e) {
    \var_dump(true);
} # 57343d
